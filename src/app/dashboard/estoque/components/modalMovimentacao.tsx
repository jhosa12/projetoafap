...
export function ModalMov({ setOpenModal, produtos, empresas, reqDadosEstoq, setModalNovo, permissoes }: DataProps) {
    ...
    return (
        <>
            ...
            <ModalConfirmar pergunta="DESEJA REALMENTE FINALIZAR ESSA MOVIMENTAÇÃO ?" handleConfirmar={()=>handleMovimentar(status)} openModal={modalConfirm} setOpenModal={()=>setModalConfirm(false)}/>
                ...
                <DialogContent className="max-w-[1000px]">
                    <div className="inline-flex gap-4 w-full items-center">
                        <Card className="w-1/2">
                            <CardHeader>
                                <CardTitle className="text-lg text-center">{arrayMov[arrayMov?.length - 1]?.produto} - {arrayMov[arrayMov?.length - 1]?.quantidade}</CardTitle>
                            </CardHeader>
                        ...
                        <Card className="w-4/5">
                            <CardHeader>
                                <CardTitle className="text-lg text-center p-2">MOVIMENTAÇÃO</CardTitle>
                            </CardHeader>
                            <CardContent className="flex flex-col gap-2">
                                    ...
                                    <Table className="text-sm text-black">
                                        ...
                                        <TableBody className="divide-y-2">
                                            {arrayMov.map((item, index) => (
                                                <TableRow className="font-semibold text-base" key={Number(item.id_produto)}>
                                                    <TableCell>{item.produto}</TableCell>
                                                    <TableCell>{item.quantidade}</TableCell>
                                                    <TableCell><button onClick={() => handleDeletarProduto(index)} className="hover:text-red-600"><HiTrash size={16} /></button></TableCell>
                                                </TableRow>
                                            ))}
                                        </TableBody>
                                    </Table>
                                ...
                                <div className="space-y-2 mt-1">
                                    ...
                                    <Select onValueChange={(val) => {
                                        const empresa = empresas.find(item => item.id === val)
                                        if (!empresa) {
                    ...
                    <div className="inline-flex w-full justify-between mt-2">
                        <Button disabled={!permissoes.includes('EST1.3')} onClick={() => { setStatus('ENTRADA'); setModalConfirm(true) }} variant={'default'}>ENTRADA</Button>
                        <Button disabled={!permissoes.includes('EST1.2')} variant={'destructive'} onClick={() => { setStatus('SAIDA'); setModalConfirm(true) }}>SAÍDA</Button>
                    </div>
                </DialogContent>
            </Dialog>
        </>
