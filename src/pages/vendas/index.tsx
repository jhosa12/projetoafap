  ...
  return (
    <>
      <Head>
        ...
        <Tabs theme={{base: 'bg-white rounded-b-lg',tabpanel:'bg-white rounded-b-lg h-[calc(100vh-100px)]',tablist:{tabitem:{base: "flex items-center  justify-center rounded-t-lg px-4 py-3 text-xs font-medium first:ml-0  disabled:cursor-not-allowed disabled:text-gray-400 disabled:dark:text-gray-500",variant:{underline:{active:{
        ...
        off:"border-b-2 border-transparent text-black hover:border-gray-700 hover:text-gray-600 "
          ...
          <Tabs.Item active={tab === 0} title="ACOMPANHAMENTO" icon={()=><FaStore className="mr-2 h-4 w-4"/>}>
             <Acompanhamento usuario={usuario?.nome??''} setores={data??[]} empresa={selectEmp} />
