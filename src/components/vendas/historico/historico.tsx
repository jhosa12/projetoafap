...
export function Historico() {
    ...
    }, Partial<CadastroRequest>>("/leads/gerarPlano")
    ...
    return (
        <div className="flex-col w-full px-2 mt-1 bg-white   ">
            {modal.filtro && <ModalFiltro
                consultores={consultores.filter((item) => item.funcao === 'PROMOTOR(A) DE VENDAS').map((item) => ({ label: item.nome, value: item.nome }))}
            ...
            <div className="inline-flex gap-4">
                `
                <Button onClick={() => setModal({ filtro: true })}  size={'sm'} variant={'outline'}>
                    <MdFilter1/>
                    FILTRAR
                    </Button>
                <Button onClick={() => setModal({ print: true })}  size={'sm'} variant={'outline'}>
                    <MdPrint/>
                    IMPRIMIR
                    </Button>
            </div>
...
export const ModalFiltro = ({ onClose, show, handleOnSubmit, register, control, handleSubmit, loading, consultores }: DataProps) => {
            ...
            <DialogContent >
                ...
                <Controller
                    ...
                    <div>
                        ...
                        <Input {...register('nome')} className="uppercase"  placeholder="NOME" />
                    </div>
