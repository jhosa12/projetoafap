...
export function GerenciarMetas({carregarDados,arrayMetas,setarMetas,arraygrupos}:DadosProps){
...
const handleGrupo=(index:number,event:React.ChangeEvent<HTMLInputElement>)=>{
    const novoArray = [...arrayMetas]
         ...
         className="block  overflow-y-auto overflow-x-auto text-sm text-left rtl:text-center border-collapse rounded-lg text-gray-400">
            ...
            <tbody>
               ...
               <tr key={index}  className={ `border-b bg-gray-800 border-gray-700 w-full hover:bg-gray-600`}>
                ...
                <input onChange={(event)=>handleLimite(index,event)}  value={new Date(item.date).toLocaleDateString()??''} className="flex bg-transparent w-full pl-2" type="text" />
                   
                </td>
                <td className="px-12 py-1 w-full ">
                    <span className="inline-flex">R$<input onChange={(event)=>handleGrupo(index,event)}  value={item.valor} className="flex bg-transparent w-full"/></span>
                </td>
