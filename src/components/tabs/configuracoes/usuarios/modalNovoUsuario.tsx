...
import { MdOutlineFileUpload, MdOutlineAdminPanelSettings } from "react-icons/md";
...
import { ActionsFunctionsApi } from "@/types/globaltypes";
...
export function ModalNovoUsuario({ setarDadosUsuario, dadosUser, setModal, show, handlePermission, handleEditarCadastro, handleNovoCadastro }: DataProps) {
    ...
    const [showPermissoes, setShowPermissoes] = useState<boolean>(false);
    ...
    return (
        ...
        <Dialog open={show} onOpenChange={setModal}>
                ...
                <div className="inline-flex w-full gap-2 ">
                    ...
                    <Card className="flex w-full flex-col gap-4 p-6">
                        <div className="flex w-full relative">
                            <div className="flex w-full h-40 justify-center cursor-pointer rounded-lg items-center border-[1px]">
                                ...
                                {dadosUser.avatarUrl && !dadosUser.image && <img className="flex w-full h-full object-cover rounded-lg" src={dadosUser.avatarUrl} alt="fotoUser" width={150} height={100}></img>}

                                {dadosUser.image && !dadosUser.avatarUrl && <img className="flex w-full h-full object-cover rounded-lg" src={`${dadosUser.image}`} alt="fotoUser" width={150} height={100}></img>}
            ...
            <ModalPermissoesUsuario
                isOpen={showPermissoes}
                onClose={() => setShowPermissoes(false)}
                permissions={dadosUser.permissoes ?? []}
                onPermissionChange={handlePermission}
                title={`Permissões - ${dadosUser.nome || 'Novo Usuário'}`}
            />
        </Dialog>
