                       ...
                       <div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%' }}>
                                        ...
                                        item.tipo === 'RECEITA' && <li key={index} style={{ display: 'flex', fontSize: '13px', justifyContent: 'space-between', width: '100%',borderBottom:'1px solid gray'}}>
                                                <span className="truncate" style={{ whiteSpace: 'nowrap',textAlign:'left' }}>{item.historico}</span>
                                                <span style={{ whiteSpace: 'nowrap',alignItems:'left' }}>{item?.mensalidade?.form_pagto}</span>
                                                <span style={{ whiteSpace: 'nowrap',textAlign:'right' }}>{Number(item.valor).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
                                        </li>
                                ))}
                                        </ul>
                        ...
                        <div>
                                ...
                                {array.sort((a, b) => a.lanc_id - b.lanc_id)?.map((item, index) => (
                                        item.tipo === 'DESPESA' && <li key={index} style={{ display: 'flex', fontSize: '13px', justifyContent: 'space-between', width: '100%',borderBottom:'1px solid gray' }}>
                                                <span className="truncate" style={{ whiteSpace: 'nowrap',textAlign:'left' }}>{item.historico}</span>
                                                <span style={{ whiteSpace: 'nowrap',textAlign:'left' }}>{Number(item.valor).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
                                        </li>
                                ))}
                                </ul>
